...
char szMesg[MAX_MESG+1] = "";          //Is this where I need 2nd time??
char zuluTime;
//char hour;
char h;
...
// Code for reading clock date
{
  ...
  RTC.readTime();
  sprintf(psz, "%d %s %d", RTC.dd, mon2str(RTC.mm, szBuf, sizeof(szBuf)-1), RTC.yyyy);        //REMOVED 
...
////////// TRYING TO FIX TEMP SENSOR
  ...
  if (isnan(hum) || isnan(temp) || isnan(fern)) {
    return;
...
///////// END TEMP SENSOR
  ...
  P.setIntensity(1);                  //Intensity - Change with time later
